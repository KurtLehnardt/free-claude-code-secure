...
# System tools the installer invokes before/through the running-process check.
...
# The fixed sha256 the stubbed sha256sum reports for every downloaded artifact.
# The generated harness manifest pins every component to it so real checksum
# verification passes; dedicated tests override individual entries to exercise
# the fail-closed / mismatch behavior.
STUB_SHA256 = "d94cc2a3e57fa534892b5235a726e7eeb7523f205a5f8f48f853bfcae7be7e33"

# Every component id the hardened install.sh may verify against the manifest.
_CHECKSUM_COMPONENT_IDS = (
    "claude-installer",
    "codex-installer",
    "pi-installer",
    "opencode-installer",
    ...
    "uv-0.11.16-aarch64-unknown-linux-gnu",
    "uv-0.11.16-x86_64-apple-darwin",
    "uv-0.11.16-aarch64-apple-darwin",
    "rtk-0.44.2-x86_64-unknown-linux-musl",
    "rtk-0.44.2-aarch64-unknown-linux-gnu",
    "rtk-0.44.2-x86_64-apple-darwin",
    "rtk-0.44.2-aarch64-apple-darwin",
    "npm-cline-3.0.55",
    "npm-dsh-0.1.0-rc.8",
)

# uv release target triples the installer may download+extract per platform.
_UV_TARGETS = (
...
def _checksums_text(overrides: dict[str, str] | None = None) -> str:
    overrides = dict(overrides or {})
    known = [
        f"{component_id}={overrides.get(component_id, STUB_SHA256)}"
        for component_id in _CHECKSUM_COMPONENT_IDS
    ]
    extra = [
        f"{component_id}={value}"
        for component_id, value in overrides.items()
        if component_id not in _CHECKSUM_COMPONENT_IDS
    ]
    header = "# Test checksum manifest generated by the installer harness."
    return "\n".join([header, *known, *extra]) + "\n"


def _uv_download_index(calls: list[str]) -> int:
    return next(
        index
        for index, call in enumerate(calls)
        if call.startswith("download:") and "astral-sh/uv/releases" in call
    )


def _npm_pack_index(calls: list[str], spec_prefix: str) -> int:
    return next(
        index
        for index, call in enumerate(calls)
        if call.startswith(f"npm:pack {spec_prefix}")
    )


def _npm_install_index(calls: list[str], tarball_substring: str) -> int:
    return next(
        index
        for index, call in enumerate(calls)
        if call.startswith("npm:install -g") and tarball_substring in call
    )
...
def _posix_npm_command() -> str:
    # The hardened installer never runs `npm install -g <name>` against the
    # registry. It fetches the pinned tarball via `npm pack ... --ignore-scripts`
    # into a temp dir, sha256-verifies it, and only then installs FROM the
    # verified local tarball. This stub models both halves hermetically: `pack`
    # writes a dummy .tgz whose name encodes the package, and `install -g`
    # detects the package from the tarball name it is handed.
    return """#!/bin/sh
echo "npm:$*" >> "$CALL_LOG"
if [ "${1:-}" = "pack" ]; then
    spec=$2
    ...
    case "$spec" in
        cline@*)
            [ "$FAIL_STEP" = "cline-pack" ] && exit 71
            tarball="cline-3.0.55.tgz"
            ;;
        @deepseek-ai/dsh@*)
            [ "$FAIL_STEP" = "dsh-pack" ] && exit 71
            tarball="deepseek-ai-dsh-0.1.0-rc.8.tgz"
            ;;
        *) exit 71 ;;
    esac
    mkdir -p "$dest"
    printf 'stub npm tarball for %s\n' "$spec" > "$dest/$tarball"
    printf '%s\n' "$tarball"
    exit 0
fi
...
def _posix_git_command() -> str:
    # Hermetic git stub for the pinned-commit install flow: models `clone`,
    # `checkout --detach <ref>`, and `rev-parse HEAD` with no network. checkout
    # records the requested ref so rev-parse echoes it back (HEAD == pinned
    # commit) and the installer's verification passes. FAIL_STEP=fcc-rev-mismatch
    # makes rev-parse report a different commit so verification aborts;
    # FAIL_STEP=fcc-clone / fcc-checkout inject clone/checkout failures.
    return """#!/bin/sh
echo "git:$*" >> "$CALL_LOG"
if [ "${1:-}" = "clone" ]; then
    [ "$FAIL_STEP" = "fcc-clone" ] && exit 92
    mkdir -p "$3"
    exit 0
fi
if [ "${1:-}" = "-C" ]; then
    ...
    case "${1:-}" in
        --detach)
            printf '%s' "$2" > "$repo_dir/.fcc-head"
            [ "$FAIL_STEP" = "fcc-checkout" ] && exit 91
            exit 0
            ;;
        HEAD)
            if [ "$FAIL_STEP" = "fcc-rev-mismatch" ]; then
                printf '0000000000000000000000000000000000000000\n'
            else
                head=$(cat "$repo_dir/.fcc-head" 2>/dev/null || true)
                printf '%s\n' "$head"
            fi
            exit 0
            ;;
...
class PosixHarness:
    ...
    def write_checksums(self, overrides: dict[str, str]) -> Path:
        """Write a manifest (all components pinned to the stub hash, then the
        given overrides applied) and return its path, for exercising the
        fail-closed / mismatch behavior of the checksum gate."""
        path = self.root / "custom-checksums.txt"
        path.write_text(_checksums_text(overrides), encoding="utf-8")
        return path
...
""",
        ...
        # Force the pgrep-less code path: the installer only reaches the
        ...
        # tools the installer touches before/through the process check are
        # exposed as SYMLINKS, never copies: copying a macOS platform binary
        # (awk, dirname, ...) invalidates its code signature and the kernel
        # SIGKILLs the copy, and copy2/copystat replays BSD file flags via
        # chflags which is rejected under a restricted sandbox.
...
case "$url:$FAIL_STEP" in
    *claude.ai*:claude-download|*chatgpt.com*:codex-download|*pi.dev*:pi-download|*opencode.ai*:opencode-download|*hermes-agent.nousresearch.com*:hermes-download|*x.ai*:grok-download|*dev.meta.ai*:muse-download|*rtk-ai*:rtk-download|*astral-sh/uv*:uv-download)
        exit 41
        ;;
esac
case "$url" in
    ...
    *astral-sh/uv/releases*)
        # Serve the pinned uv release tarball for the requested target triple.
        source="$FAKE_FIXTURES/${url##*/}"
        ;;
    *) exit 42 ;;
esac
cp "$source" "$output"
...
""",
    ...
    # Pinned uv release tarballs (one per target triple). Each holds a single
    # uv-<target>/uv entry -- the uv stub -- matching the layout install.sh
    # validates and extracts via install_uv_pinned.
    uv_stub = _posix_uv_command("0.11.28").encode()
    for uv_target in _UV_TARGETS:
        with tarfile.open(fixtures / f"uv-{uv_target}.tar.gz", "w:gz") as archive:
            metadata = tarfile.TarInfo(f"uv-{uv_target}/uv")
            metadata.mode = 0o755
            metadata.size = len(uv_stub)
            archive.addfile(metadata, io.BytesIO(uv_stub))
    _write_executable(
...
""",
    ...
    _write_executable(bin_dir / "git", _posix_git_command())
    npm_prefix = tmp_path / "npm-prefix"
    npm_prefix.mkdir()
    _write_executable(bin_dir / "npm", _posix_npm_command())
    # The stub reports the same sha256 for every artifact; the harness manifest
    # pins every component to it so real verification passes. Per-component
    # mismatches are driven by manifest overrides, not by lying here.
    _write_executable(
        bin_dir / "sha256sum",
        f"""#!/bin/sh
...
""",
    ...
    checksums_path = tmp_path / "install.checksums"
    checksums_path.write_text(_checksums_text(), encoding="utf-8")
    ...
    env.update(
        {
            ...
            "CLAUDE_CONFIG_DIR": "",
            "FCC_CHECKSUMS_FILE": str(checksums_path),
            "FAIL_STEP": "",
        }
    )
    env.pop("XDG_BIN_HOME", None)
    return PosixHarness(tmp_path, bin_dir, fixtures, tool_bin, log, env, checksums_path)
...
def test_install_sh_fresh_install_is_verified(posix_harness: PosixHarness) -> None:
    ...
    # Cline/DeepSeek Harness are fetched via `npm pack` + sha256 verify, then
    # installed from the verified local tarball (never `npm install -g <name>`).
    assert (
        _npm_pack_index(calls, "cline@3.0.55")
        < _npm_install_index(calls, "cline-3.0.55.tgz")
        < calls.index("cline:--version")
    )
    assert calls.index("hermes-install:--non-interactive --skip-setup") < calls.index(
        "hermes:--version"
    )
    assert (
        _npm_pack_index(calls, "@deepseek-ai/dsh@0.1.0-rc.8")
        < _npm_install_index(calls, "dsh-0.1.0-rc.8.tgz")
        < calls.index("dsh:--version")
    )
    assert calls.index("grok-install") < calls.index("grok:--version")
    assert calls.index("muse-install") < calls.index("muse:--version")
    # uv is installed from the pinned astral-sh/uv release tarball, not a script.
    assert _uv_download_index(calls) < calls.index("uv:--version")
...
def test_install_sh_stops_when_selected_hermes_install_fails(
    ...
    assert not any("astral-sh/uv" in call for call in posix_harness.calls())
...
def test_install_sh_replaces_mismatched_dsh_preview(
    ...
    calls = posix_harness.calls()
    assert _npm_pack_index(calls, "@deepseek-ai/dsh@0.1.0-rc.8") < _npm_install_index(
        calls, "dsh-0.1.0-rc.8.tgz"
    )
...
def test_install_sh_installs_and_configures_rtk_for_selected_agents(
    ...
    assert calls.index("rtk:init --global --opencode:telemetry=1") < _uv_download_index(
        calls
    )
...
def test_install_sh_stops_when_rtk_setup_fails(
    ...
    if failure == "rtk-checksum":
        # A checksum failure is a genuine manifest mismatch: the downloaded RTK
        # archive hashes to the stub value, which no longer matches the pinned
        # entry, so verification aborts before extraction.
        manifest = posix_harness.write_checksums(
            {"rtk-0.44.2-x86_64-unknown-linux-musl": "0" * 64}
        )
        result = posix_harness.run("--rtk", "--checksums", str(manifest))
    else:
...
def test_install_sh_continues_when_pi_is_not_installed(
    ...
    assert any("astral-sh/uv/releases" in call for call in calls)
...
def test_install_sh_replaces_obsolete_uv(posix_harness: PosixHarness) -> None:
    ...
    assert any("astral-sh/uv/releases" in call for call in posix_harness.calls())
...
def test_install_sh_stops_without_success_on_each_failure(
    ...
    forbidden = {
        ...
        "pi-verify": "astral-sh/uv",
        "opencode-download": "opencode-install",
        "opencode-install": "opencode:--version",
        "opencode-verify": "astral-sh/uv",
        "cline-install": "cline:--version",
        "cline-verify": "astral-sh/uv",
        "uv-download": "uv:tool install",
        "uv-verify": "uv:tool install",
        "fcc-clone": "uv:tool install",
...
# ---------------------------------------------------------------------------
# Supply-chain hardening: checksum gate + pinned-commit verification
# ---------------------------------------------------------------------------


def test_install_sh_fails_closed_on_replace_me_without_allow_unpinned(
    posix_harness: PosixHarness,
) -> None:
    # A REPLACE_ME (unpinned) component is refused before it is executed: the
    # first downloaded installer aborts the run, and its script never runs.
    (posix_harness.bin_dir / "opencode").unlink()
    manifest = posix_harness.write_checksums({"claude-installer": "REPLACE_ME"})

    result = posix_harness.run("--checksums", str(manifest))

    assert result.returncode != 0
    assert 'No pinned sha256 for component "claude-installer"' in result.stderr
    calls = posix_harness.calls()
    assert "claude-install" not in calls
    assert not any("astral-sh/uv" in call for call in calls)


def test_install_sh_refresh_checksums_prints_hashes_without_executing(
    posix_harness: PosixHarness,
) -> None:
    # --refresh-checksums downloads each component, prints id=sha256 lines, and
    # exits: nothing is executed and the manifest is left byte-for-byte intact.
    manifest_before = posix_harness.checksums_path.read_bytes()

    result = posix_harness.run("--refresh-checksums")

    assert result.returncode == 0, result.stderr
    assert f"claude-installer={STUB_SHA256}" in result.stdout
    assert f"codex-installer={STUB_SHA256}" in result.stdout
    assert f"npm-cline-3.0.55={STUB_SHA256}" in result.stdout
    assert "uv-0.11.16-x86_64-unknown-linux-gnu=" in result.stdout
    calls = posix_harness.calls()
    # Downloads happen (to hash), but nothing is executed or installed.
    assert any(call.startswith("download:") for call in calls)
    assert "claude-install" not in calls
    assert not any(call.startswith("uv:tool install") for call in calls)
    assert not any(call.startswith("git:") for call in calls)
    # The manifest is never edited by refresh mode.
    assert posix_harness.checksums_path.read_bytes() == manifest_before


def test_install_sh_aborts_on_checksum_mismatch(
    posix_harness: PosixHarness,
) -> None:
    # A real (non-REPLACE_ME) pin that does not match the downloaded bytes aborts
    # before the component runs.
    (posix_harness.bin_dir / "opencode").unlink()
    manifest = posix_harness.write_checksums({"claude-installer": "de" * 32})

    result = posix_harness.run("--checksums", str(manifest))

    assert result.returncode != 0
    assert "Checksum verification failed for Claude Code" in result.stderr
    assert "claude-install" not in posix_harness.calls()


def test_install_sh_aborts_on_fcc_commit_mismatch(
    posix_harness: PosixHarness,
) -> None:
    # If the checked-out HEAD does not equal the pinned commit, the install
    # aborts before handing the checkout to uv.
    (posix_harness.bin_dir / "opencode").unlink()

    result = posix_harness.run(fail_step="fcc-rev-mismatch")

    assert result.returncode != 0
    assert "commit verification failed" in result.stderr
    assert not any(
        call.startswith("uv:tool install --force") for call in posix_harness.calls()
    )


def test_install_sh_allow_unpinned_still_aborts_on_real_pin_mismatch(
    posix_harness: PosixHarness,
) -> None:
    # --allow-unpinned relaxes only REPLACE_ME/missing pins (with a warning
    # banner); a real pin that mismatches still fails closed.
    (posix_harness.bin_dir / "opencode").unlink()
    manifest = posix_harness.write_checksums({"claude-installer": "de" * 32})

    result = posix_harness.run("--checksums", str(manifest), "--allow-unpinned")

    assert result.returncode != 0
    assert "SECURITY WARNING: --allow-unpinned is set" in result.stderr
    assert "Checksum verification failed for Claude Code" in result.stderr
    assert "claude-install" not in posix_harness.calls()
